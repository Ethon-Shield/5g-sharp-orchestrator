...
function logging {
  ...
  if [[ "${component}" == "GNB " ]]; then
    case $severity in
      "INFO") ssh ${SHARP_ORCHESTRATOR_USERNAME}@${SHARP_ORCHESTRATOR_IP_ADDRESS} "printf \"$(date +'%Y-%m-%d %H:%M:%S') [INFO ] [${component}] [${ip_address}] - ${output_text} \n\" >> ${LOG_FILE}" & ;;
      "WARNING") ssh ${SHARP_ORCHESTRATOR_USERNAME}@${SHARP_ORCHESTRATOR_IP_ADDRESS} "printf \"${WARNING}$(date +'%Y-%m-%d %H:%M:%S') [WARN ] [${component}] [${ip_address}] [${file_name}] - ${output_text}${NC}\n\" >> ${LOG_FILE}" & ;;
      "ERROR") ssh ${SHARP_ORCHESTRATOR_USERNAME}@${SHARP_ORCHESTRATOR_IP_ADDRESS} "printf \"${ERROR}$(date +'%Y-%m-%d %H:%M:%S') [ERROR] [${component}] [${ip_address}] [${file_name}] - ${output_text}${NC}\n\" >> ${LOG_FILE}" & ;;
      "FATAL") ssh ${SHARP_ORCHESTRATOR_USERNAME}@${SHARP_ORCHESTRATOR_IP_ADDRESS} "printf \"${FATAL}$(date +'%Y-%m-%d %H:%M:%S') [FATAL] [${component}] [${ip_address}] [${file_name}] - ${output_text}${NC}\n\" >> ${LOG_FILE}" & ;;
      "DEBUG") 
        if [[ "${DEBUG}" == "true" ]]; then 
          ssh ${SHARP_ORCHESTRATOR_USERNAME}@${SHARP_ORCHESTRATOR_IP_ADDRESS} "printf \"${DEBUG_C}$(date +'%Y-%m-%d %H:%M:%S') [DEBUG] [${component}] [${ip_address}] - ${output_text}${NC}\n\" >> ${LOG_FILE}" &
        fi
      ;;
    esac
