...
#logging "INFO" "NMAN" "${SHARP_ORCHESTRATOR_IP_ADDRESS}" "## INITIALIZING ##"
...
if [[ "${DEPLOY_NRCORE_ONLY}" == "false" ]]; then
  ...
  if [[ ${check_gnb} -gt 0 ]]; then
    ...
    if [[ ${stopping_gnb} -eq 0 ]]; then
      logging "DEBUG" "NMAN" "${SHARP_ORCHESTRATOR_IP_ADDRESS}" "GNB stopped correctly"
    else
...
if [[ "${NRCORE_TECH}" == "OAI" ]]; then
 ...
 logging "DEBUG" "NMAN" "${SHARP_ORCHESTRATOR_IP_ADDRESS}" "Undeploying OAI NRCORE instance. It might take some time..."
 maxloop=0
 ...
 if [[ "${is_core_down}" -eq 1 ]];then
   logging "DEBUG" "NMAN" "${SHARP_ORCHESTRATOR_IP_ADDRESS}" "OAI NRCORE instance undeployed successfully"
 else
  ...
  if [[ "${are_open5gs_processes_running}" -eq 0 ]]; then
    logging "DEBUG" "NMAN" "${SHARP_ORCHESTRATOR_IP_ADDRESS}" "Open5gs NRCORE instance undeployed successfully"
  else
...
if [[ ${is_network_undeployed} -eq 1 ]]; then
	...
	logging "INFO" "NMAN" "${SHARP_ORCHESTRATOR_IP_ADDRESS}" "## SHARP ORCHESTRATOR INITIALIZED ##"
else
